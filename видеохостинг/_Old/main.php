...
$_SESSION['add_razdel_text']='streams';

echo'<div align=right>';
if(@$_SESSION['user_id']<>""){echo @$_SESSION['user_nick'].' &nbsp;&nbsp;&nbsp; <a href="del_cookie.php">Выйти</a></div>';}
if(@$_SESSION['user_id']==2){
	$STH = $DBH->query("SELECT id FROM streams WHERE check_=0");
$u = $STH->rowCount();
if($u>0){echo'Ожидают модерации: <a href="javascript:top.moderation();">'.$u.'</a><br><br>';}
}
echo'<hr><br>';

echo'<table border=0 width=945><tr><td align=center>';
if(@$_GET['razdel']=="undefined" OR @$_GET['razdel']==""){include("news.php");}
...
try{$STH = $DBH->query("SELECT id,nazv,about,code FROM streams WHERE razdel='".@$_GET['razdel']."' AND cat_id='".@$_GET['cat']."' AND subcat_id='".@$_GET['subcat']."' ORDER BY id DESC LIMIT 100");
	...
	while($row = $STH->fetch()) {  
		...
		/*
		echo mb_substr($code,$pos+6,$pos2-$pos-8);
		*/
