...
while($row = $STH->fetch()) {  
	$cid=$row['id'];
	$cnazv=$row['nazv'];
	echo '<a href="javascript:main(\''.$cid.'\',\'\',\'\',\''.@$_GET['razdel'].'\');"><img width=200 height=150 src="images/categories/'.$cid.'.png"></a> ';
...
echo'<div align=center><font class=style1>'.$nazv.'</style><br><br><table border=0 width=800><tr><td width=240><img src=images/categories/'.@$_GET['cat'].'.png></td><td valign=top>'.$about.'</td></tr></table></div><br>';
}
if(@$_GET['cat']<>"" AND @$_GET['cat']<>"undefined"){
	...
	while($row = $STH->fetch()) {  
		$uid=$row['id'];
		$unazv=$row['nazv'];
		echo '<a href="javascript:main(\''.@$_GET['cat'].'\',\''.$uid.'\',\'\',\''.@$_GET['razdel'].'\');"><img src="images/subcategories/'.$uid.'.png"></a> ';
	}}catch(PDOException $e){SQLErrors($e,'#error_02#');}
...
try{$STH = $DBH->query("SELECT id,nazv,about,code FROM streams WHERE razdel='".@$_GET['razdel']."' AND cat_id='".@$_GET['cat']."' AND subcat_id='".@$_GET['subcat']."' ORDER BY id DESC LIMIT 100");
	...
	while($row = $STH->fetch()) {  
		...
		$about=$row['about'];
		$code=$row['code'];
		echo'<a href="javascript:see(\''.$id.'\');"><img src=images/elements/'.$id.'.png></a> ';
