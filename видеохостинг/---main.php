...
try{$STH = $DBH->query("SELECT id,nazv,about,code FROM streams WHERE razdel='".@$_GET['razdel']."' AND cat_id='".@$_GET['cat']."' AND subcat_id='".@$_GET['subcat']."' ORDER BY id DESC LIMIT 100");
	...
	while($row = $STH->fetch()) {  
		...
		$about=$row['about'];
		$code=$row['code'];
		echo'<a href="javascript:see(\''.$id.'\');"><img src=images/elements/'.$id.'.png onmouseover="this.src=\'images/elements/'.$id.'_.png\'" onmouseout="this.src=\'images/elements/'.$id.'.png\'"></a> ';
